---
id: PR128-REVIEW-INSPECTION-BEHAVIOUR-HAS-NO-DESIGN-SENTENCE
severity: P2
...
`object_exists` and `unreachable_objects` changed public behaviour with no `design/` sentence -> the sole-authority rule and §13's same-change requirement were not met -> a reviewer had no living authority to check the behaviour against
