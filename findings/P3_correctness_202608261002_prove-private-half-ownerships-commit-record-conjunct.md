...
A withheld-mutation-catalogue entry targeting `prove_private_half_ownership`'s commit-record-absence conjunct (`src/workspace_manager.rs:6810`). On re-measurement against the shipped
tree it comes back `KILLED`, and the killing assertion is confirmed still present, byte-identical
or near-identical. The production check text is byte-identical to what it was at catalogue time, so this one leans hard toward the equivalent-mutant reading.
