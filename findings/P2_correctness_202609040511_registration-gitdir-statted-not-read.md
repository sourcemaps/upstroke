---
id: PR128-REVIEW2-REGISTRATION-GITDIR-STATTED-NOT-READ
severity: P2
disposition: deferred
category: correctness
pr: 128
reviewed_sha: f161c9e8555c88474f09f89f095737346070d334
location: src/workspace_manager.rs:3249
...
the registration scan called `symlink_metadata` on each `gitdir` and never read it -> a `gitdir` at mode 000 stats fine, and `git worktree list` exits 0 and omits that worktree (measured on git 2.43), as does an empty `gitdir` -> `record_for` answered `None`, `add_state` `Unregistered` and the classifier `ObjectResidue::None` for a registration this process could not read; the pass-1 test chmod-ed the containing directory, so it missed the case
