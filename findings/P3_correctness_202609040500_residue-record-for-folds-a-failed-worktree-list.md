---
id: PR128-RESIDUE-RECORD-FOR-FOLDS-A-FAILED-WORKTREE-LIST
severity: P3
disposition: deferred
category: correctness
pr: 128
reviewed_sha: 80843302a8367e607e54f181ef592c02ca5a089f
location: src/workspace_manager.rs:3063
...
`record_for` answers `None` when `git worktree list --porcelain -z` exits non-zero -> the classifier's three add sites read `None` as not registered, so the after phase is absent and `RegisteredUnpopulatedWorktree` is absent -> `classify_object_residue` answers `None` for a repository Git could not open, the class no action recovers; a spawn failure is propagated, a failure that speaks is not; no production caller reaches the add sites today
